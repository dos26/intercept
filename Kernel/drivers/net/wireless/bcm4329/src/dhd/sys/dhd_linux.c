...
//<<SecFeature.Vinsq
extern uint32 extra_wakeup_stat;
extern uint32 extra_eint0pend;
extern irqreturn_t wlan_oob_irq(int irq, void *dev_id); /* bcmsdh_linux.c */
#define WAKEUP_STAT_EINT 0x00000001
#define EINT0PEND_WLAN 0x00000002
//SecFeature.Vinsq>>
...
static int dhd_sleep_pm_callback(struct notifier_block *nfb, unsigned long action, void *ignored)
{
	...
	switch (action)
	{
		...
		case PM_POST_SUSPEND:
			...
			//<<SecFeature.Vinsq
                   if ((extra_eint0pend & EINT0PEND_WLAN ) && (extra_wakeup_stat & WAKEUP_STAT_EINT )) {
		      DHD_TRACE(("%s: Enter\n here", __FUNCTION__));
                      wlan_oob_irq(0, NULL);
                   }
                   //SecFeature.Vinsq>>			
